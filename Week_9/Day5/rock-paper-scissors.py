...
class Rps(object):
    ...
    def play_game(self):
        while self.winner is None:
            # Insert a pause so that the player has time to brace himself before the action starts.
            self.pause()
            plyr = Game()
            # Call player_hand method to get the player's hand.
            hand_p = plyr.get_user_item()
    ...
    def display_winner(self, winner, loser):
        print("{} has lost. {} has won. Long live {}!".format(
            self.loser, self.winner, self.winner))

    def pause(self):    # Requires the human to hit enter before continuing.
        if self.human:
            input(
                "Press 'enter' when you're ready to play against Alfred (the computer).")
